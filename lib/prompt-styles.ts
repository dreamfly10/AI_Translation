...
/**
 * Some parts of the app import RewritingLevel from this module.
 * Keep it here to satisfy those imports even if you don't use it everywhere yet.
 */
...
/**
 * Typed keys helper.
 * Use this instead of Object.keys(styleArchetypes) in UI code to avoid TS 'string' issues.
 */
...
export function getStyleUserPrompt(chineseTranslation: string, style: StyleArchetype): string {
  ...
  return `Based on the following translated article, write an insightful interpretation following the "${config.name}" style.
